...
public class Rasterer {
    ...
    public Map<String, Object> getMapRaster(Map<String, Double> params) {
        x0 = params.get("ullon"); y0 = params.get("ullat");
        x3 = params.get("lrlon"); y3 = params.get("lrlat");
        if (x3 <= x0 || y0 <= y3) {
        ...
        String[][] imageFiles = new String[Math.abs(filezuoshangy - fileyouxiay) + 1 ]
                [Math.abs(fileyouxiax - filezuoshangx) + 1];
        for (int i = 0; i <= fileyouxiay - filezuoshangy; i++) {
            for (int j = 0; j <= fileyouxiax - filezuoshangx; j++) {
                imageFiles[i][j] = "d" + depth + "_x"
                        + (j + filezuoshangx)  + "_y" + (i + filezuoshangy) + ".png";
            }
        }
        fillResults(imageFiles);
        return results;
    }


    private boolean checkWithinWorld(double x, double y) {
        return x <= ROOT_LRLON && x >= ROOT_ULLON && y <= ROOT_ULLAT && y >= ROOT_LRLAT;
    }

    private void matchDepth(double w) {
        double queryResolution = (x3 - x0) / w;
        if (queryResolution <= resolutionLevel[7]) {
            ...
            for (int i = 0; i <= 6; i++) {
                if (resolutionLevel[i] >= queryResolution && resolutionLevel[i + 1]
                        < queryResolution) {
                    depth = i + 1;
                    break;
                }
    ...
    private void fillResults(String[][] imageFiles) {
